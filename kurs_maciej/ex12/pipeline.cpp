...
void Pipeline::InitPerspectiveProj(Matrix4f &m) const
{
  ...
  const float NearZ      = m_persProj.NearZ;
  const float FarZ       = m_persProj.FarZ;
  const float RangeZ     = NearZ - FarZ;
  const float tanHalfFOV = tanf(torad(m_persProj.FOV / 2.0f));
  ...
  m.m[2][0] = 0.0f; m.m[2][1] = 0.0f; m.m[2][2] = (-NearZ-FarZ)/RangeZ; m.m[2][3] = 2.0f*NearZ*FarZ/RangeZ;
...
const Matrix4f *Pipeline::GetTransformation()
{
  ...
  Matrix4f scale, rotate, translate, perspective;

  InitScale(scale);
  InitRotate(rotate);
  InitTranslate(translate);
  InitPerspectiveProj(perspective);

  m_transformation = perspective * translate * rotate * scale;
